...
export async function getAlertById(db: D1Database, alertId: string): Promise<KVAlert | null> {
  try {
...
export async function getUserAlerts(db: D1Database, userId: string): Promise<KVAlert[]> {
  try {
    const result = await db
...
export async function getAlertByUserAndId(db: D1Database, userId: string, alertId: string): Promise<KVAlert | null> {
  try {
    const result = await db
...
export async function deleteAlert(db: D1Database, alertId: string): Promise<boolean> {
  try {
    const result = await db
